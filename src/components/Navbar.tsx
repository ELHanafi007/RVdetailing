      ...
      <div className="container mx-auto flex items-center justify-center">
        <motion.div
          animate={{ 
            ...
            backgroundColor: scrolled ? "rgba(var(--background-rgb), 0.4)" : "transparent",
            borderColor: scrolled ? "rgba(var(--foreground-rgb), 0.15)" : "rgba(var(--foreground-rgb), 0.1)",
          }}
          ...
          <div className="hidden md:flex items-center space-x-12">
            {navLinks.map((link) => (
              ...
              >
                <Link
                  href={link.href}
                  className="flex items-center gap-1 text-[10px] uppercase tracking-[0.2em] font-black text-apple-gray hover:text-foreground transition-all duration-300 py-4"
                >
                  {link.name}
                ...
                {link.dropdown && (
                  <AnimatePresence>
                    {activeDropdown === link.name && (
                      ...
                      >
                        <div className="bg-background/90 border border-foreground/10 backdrop-blur-xl rounded-3xl p-6 min-w-[200px] shadow-2xl">
                          <div className="flex flex-col space-y-4">
                            {link.dropdown.map((item) => (
                              <Link
                                key={item.name}
                                href={item.href}
                                className="text-[10px] uppercase tracking-[0.15em] font-bold text-apple-gray hover:text-foreground transition-colors"
                              >
                                {item.name}
                              </Link>
                            ))}
            ...
            <Link
              href="#contact"
              className="px-6 py-2 bg-foreground text-background text-[10px] font-black uppercase tracking-[0.2em] rounded-full hover:opacity-90 transition-all"
            >
              Inquire
            </Link>
            <ThemeToggle />
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed inset-0 top-0 bg-background/95 backdrop-blur-3xl z-40 md:hidden flex flex-col items-center justify-center p-12 space-y-12 pointer-events-auto"
          >
            {navLinks.map((link) => (
              <div key={link.name} className="text-center">
                <Link
                  ...
                  className="text-5xl font-black tracking-tighter text-foreground hover:text-apple-gray transition-colors"
                >
                  {link.name}
                </Link>
              </div>
