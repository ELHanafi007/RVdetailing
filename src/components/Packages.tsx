...
export function Packages() {
  return (
    <section id="packages" className="py-48 bg-background px-6">
        ...
        <div className="flex flex-col lg:flex-row items-end justify-between mb-32 gap-12">
          <div className="max-w-4xl">
            <motion.h2 
              ...
              viewport={{ once: true }}
              className="text-foreground text-6xl md:text-8xl lg:text-9xl font-black tracking-tighter"
            >
              INVESTMENT <br /> <span className="text-apple-gray">GUIDE.</span>
            </motion.h2>
          </div>
          <div className="max-w-xs pb-4">
             <p className="text-apple-gray font-medium text-lg leading-relaxed border-l border-foreground/20 pl-6">
          ...
          {packages.map((pkg, index) => (
            <motion.div
              ...
              className={`min-w-[85vw] md:min-w-0 p-8 md:p-12 rounded-[2.5rem] md:rounded-[3rem] transition-all duration-500 flex flex-col h-full snap-center ${
                pkg.popular ? "bg-foreground text-background md:scale-[1.02] z-10 shadow-2xl" : "bg-foreground/5 text-foreground border border-foreground/5"
              }`}
            >
              <div className="mb-8 md:mb-12">
                <h3 className="text-3xl md:text-4xl font-black tracking-tight mb-2 md:mb-4">{pkg.name}</h3>
                <p className={`${pkg.popular ? "opacity-70" : "text-apple-gray"} font-medium text-sm md:text-base`}>{pkg.description}</p>
              </div>

              <div className="mb-8 md:mb-16">
                <div className="flex items-baseline gap-2">
                  <span className="text-5xl md:text-7xl font-black tracking-tighter">{pkg.price}</span>
                  <span className={`text-xs md:text-sm font-bold uppercase tracking-widest ${pkg.popular ? "opacity-50" : "text-apple-gray"}`}>/ft</span>
                </div>
              </div>
              ...
              <ul className="space-y-4 md:space-y-6 mb-10 md:mb-16 flex-grow">
                {pkg.features.map((feature, i) => (
                  <li key={i} className="flex items-center gap-3 md:gap-4 font-bold text-sm md:text-sm tracking-tight">
                    <Check size={16} className={pkg.popular ? "text-background" : "text-foreground"} />
                    {feature}
                  </li>
                ))}
              </ul>
